...
# -*- coding: utf-8 -*-
"""
@ File  : TwoCategoryColsDateTimeStatistics.py
@ Author: WangJun
@ Date  : 2019-12-05
@ Desc  : 不同类别时间间隔统计：按照类别分组，分别计算不同组中相邻日期间的时间间隔，对时间间隔进行统计规则计算（统计规则：mean、max、min、count、sum、std）
"""
import numpy as np
import pandas as pd
...
class TwoCategoryColsDateTimeStatistics(DService):

    def __init__(self):
        pass

    def __calculate_2_category_time_previous_statistics(self, df, category_col1, category_col2, id_col, datetime_col,
                                                        statistics_methods):
        """
        按照id列和日期列升序排序，按照类别分组，分别计算不同组中相邻日期间的时间间隔，对时间间隔进行统计规则计算
    ...
    def process(self, dicInput, dicParams):
        ...
        outputDf = self.__calculate_2_category_time_previous_statistics(inputDf, category_col1, category_col2, id_col,
                                                                        datetime_col, statistics_methods)
