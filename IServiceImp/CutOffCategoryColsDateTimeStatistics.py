...
# -*- coding: utf-8 -*-
"""
@ File  : CutOffCategoryColsDateTimeStatistics.py
@ Author: WangJun
@ Date  : 2019-12-05
@ Desc  : 指定时间段内的类别时间间隔统计：按照统计时间段切割数据，计算相邻日期间的时间间隔，对时间间隔进行统计规则计算（统计规则：mean、max、min、count、sum、std）
"""
import numpy as np
import pandas as pd
...
class CutOffCategoryColsDateTimeStatistics(DService):

    def __init__(self):
        pass

    def __calculate_cutoff_category_time_previous_statistics(self, df, category_col, id_col, datetime_col,
                                                             cutoff_time, time_window, statistics_methods):
        """
        按照统计时间段切割数据，按照id列和日期列升序排序，计算相邻日期间的时间间隔，对时间间隔进行统计规则计算
    ...
    def process(self, dicInput, dicParams):
        ...
        outputDf = self.__calculate_cutoff_category_time_previous_statistics(inputDf, category_col, id_col,
