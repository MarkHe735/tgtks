...
class KFlodSupervisedCategoryColumnEncodingApplyService(DService):
    """
     功能        有监督类别列编码应用服务
    ...
    def process(self, dicInput, dicParams):
        ...
        for colName in data.columns:
            ...
            for i in range(len(methodList)):
                ...
                for j in range(k):
                    model = model_list[j]
                    data_encoded_df_flod = model.transform(X_col[X_col.index % k == j])
                    data_encoded_df_flod.columns = [colName + '__' + method]
                    data_encoded_df_flod_list.append(data_encoded_df_flod)
                data_encoded_df = pd.concat(data_encoded_df_flod_list, axis=0)
