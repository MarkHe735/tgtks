...
# -*- coding: utf-8 -*-
"""
@ File  : CategoryColsLabelStatistics.py
@ Author: WangJun
@ Date  : 2019-12-05
@ Desc  : 类别列标签数值统计：类别列按照标签分组后，对数值列做统计规则计算（统计规则：mean、max、min、count、sum、std）
...
class CategoryColsLabelStatistics(DService):

    def __init__(self):
        pass

    def __catetory_num_statistics(self, df, category_col, num_cols, id_col, label_col, statistics_methods):
        """
        对输入数据集，对类别列按照标签分组后，对数值列做统计规则计算（统计规则：mean、max、min、count、sum、std），
        如生成男性在好客户中的平均身高
        :param df:
        :param category_col:
        :param num_cols:
        ...
        dataframe = df[[id_col, category_col, label_col] + num_cols]
        ...
        es = es.normalize_entity(base_entity_id=base_entity,
                                 ...
                                 index=category_col)
        es[base_entity][label_col].interesting_values = df[label_col].unique().tolist()
    ...
    def process(self, dicInput, dicParams):
        ...
        outputDf = self.__catetory_num_statistics(inputDf, category_col, num_cols, id_col, label_col, statistics_methods)
